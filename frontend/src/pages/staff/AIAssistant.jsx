...
export default function AIAssistant() {
  const [messages, setMessages] = useState([
    { role: 'assistant', text: 'Hi! I can help you with class attendance, spotting at-risk students, and reports. What do you need?' }
  ])
  const [input, setInput] = useState('')
  const [loading, setLoading] = useState(false)

  const quickPrompts = [
    { label: '📋 Show today\'s absentees', prompt: 'Show absentees today' },
    { label: '⚠️ At-risk students', prompt: 'Which students are at risk of falling below 75%' },
    { label: '📊 Class report', prompt: 'Generate attendance report for my class' },
  ]
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm">Get help with attendance management</p>
      </div>
